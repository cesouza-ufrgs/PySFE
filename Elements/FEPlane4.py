...
def update_B_plane4(dxidX):


    B[0,0] = dphidxi[0,0] * dxidX[0,0] + dphidxi[1,0] * dxidX[0,1]
    B[0,2] = dphidxi[0,1] * dxidX[0,0] + dphidxi[1,1] * dxidX[0,1]
    B[0,4] = dphidxi[0,2] * dxidX[0,0] + dphidxi[1,2] * dxidX[0,1]
    B[0,6] = dphidxi[0,3] * dxidX[0,0] + dphidxi[1,3] * dxidX[0,1]
    
    B[1,1] = dphidxi[0,0] * dxidX[1,0] + dphidxi[1,0] * dxidX[1,1]
    B[1,3] = dphidxi[0,1] * dxidX[1,0] + dphidxi[1,1] * dxidX[1,1]
    B[1,5] = dphidxi[0,2] * dxidX[1,0] + dphidxi[1,2] * dxidX[1,1]
    B[1,7] = dphidxi[0,3] * dxidX[1,0] + dphidxi[1,3] * dxidX[1,1]    
...
def postprocess_plane4(data, u_gl):    
  
    print('postprocessing Plane4')
    ...
    mainstress = np.zeros((data.n_elem,3))
    mainvec1 = np.zeros((data.n_elem,3))
    mainvec2 = np.zeros((data.n_elem,3))
    mainvec3 = np.zeros((data.n_elem,3))
    ...
    for i in range (0,data.n_elem):
        ...
        dXdxi  = np.dot(dphidxi,X)

        dxidX = np.linalg.inv(dXdxi)            
 
        update_B_plane4(dxidX)
        ...
        mainstress[i,:] = eigenvalues
        mainvec1[i,:] = eigenvectors[:,0]
        mainvec2[i,:] = eigenvectors[:,1]
        mainvec3[i,:] = eigenvectors[:,2]
    ...
    Results.mainstress = mainstress
        

    Results.mainvec1 = mainvec1
    Results.mainvec2 = mainvec2
    Results.mainvec3 = mainvec3
